    ...
    <Box>
      {isLoading ? (
        ...
        <>
          ...
          <CaptionCarousel
            // cards={nationalData.map(_handleRenderCarousel)}
            cards={nationalData.map((row: any, i: number) => (
              <div key={i}>{_handleRenderCarousel(row, i)}</div>
            ))}
            carouselProps={{
              display: 'block',
          ...
          <VStack spacing={10} my={10} mx={8} justifyContent={'center'}>
            <Text fontWeight={'bold'} fontSize={{ base: '2xl', md: '3xl' }}>
              OBJETIVOS
            </Text>
            <Image
              src={
                'https://res.cloudinary.com/df5nwnlnu/image/upload/v1671075067/observatorio/PIEZAS%20GR%C3%81FICAS%20-%20OBSERVATORIO%20JOVEN/POL%C3%8DTICA%20NACIONAL%20DE%20LA%20JUVENTUD/Infograf%C3%ADa_con_iconos_de_los_objetivos_de_la_pol%C3%ADtica-01_nqsdpr.jpg'
              }
              alt={'infografia politica nacional'}
            />
            <Box width={'100%'}>
            ...
            <Image
              src={firstValue.url_image}
              alt={'politica nacional detalle'}
            />
